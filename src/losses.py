...
class WeightedCrossEntropyLoss(nn.CrossEntropyLoss):
    ...
    def weights(self, outputs, encoded_targets):
        ...
        print(softmax_outputs.gather(1, torch.argmax(encoded_targets, dim=1).unsqueeze(1)).squeeze(1))
        print(softmax_outputs.gather(1, torch.argmax(softmax_outputs, dim=1).unsqueeze(1)).squeeze(1))
        weights = softmax_outputs.gather(1, torch.argmax(encoded_targets, dim=1).unsqueeze(1)).squeeze(1) - softmax_outputs.gather(1, torch.argmax(softmax_outputs, dim=1).unsqueeze(1)).squeeze(1)/torch.exp(self.alpha) + torch.exp(self.alpha)
        print(weights)
        return weights
