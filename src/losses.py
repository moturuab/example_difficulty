...
class WeightedCrossEntropyLoss(nn.CrossEntropyLoss):
    ...
    def weights(self, outputs, encoded_targets, m=0):
        ...
        if not m:
            ...
            weights = self.sigmoid(self.alpha*correct_outputs - max_outputs)**2 #**self.alpha
            ...
            weights = self.sigmoid(-(self.beta*correct_outputs - max_outputs))**2 #**self.beta
            print(torch.min(weights))
            print(torch.max(weights))
            #weights = torch.where(weights > 0.5, 0, weights) # change 1
        return weights
