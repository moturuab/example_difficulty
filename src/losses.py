...
class WeightedCrossEntropyLoss(nn.CrossEntropyLoss):
    ...
    def weights(self, outputs, encoded_targets, m=0):
        ...
        if not m:
            ...
            weights = self.sigmoid(self.alpha*correct_outputs - max_outputs + torch.log(self.alpha)) #**self.alpha
            ...
            weights = self.sigmoid(-(self.beta*correct_outputs - max_outputs + torch.log(self.beta))) #**self.beta
            print(torch.min(weights))
            print(torch.max(weights))
            #weights = torch.where(weights > torch.max(weights)-0.1, 0, weights)
            #print(torch.min(weights))
            #print(torch.max(weights))
        return weights
