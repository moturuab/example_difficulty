...
class WeightedCrossEntropyLoss(nn.CrossEntropyLoss):
    ...
    def weights(self, outputs, encoded_targets, m=0):
        ...
        if not m:
            ...
            weights = self.sigmoid(self.alpha*correct_outputs - max_outputs+1/self.alpha) #**self.alpha
            ...
            weights = self.sigmoid(-(self.beta*correct_outputs - max_outputs)+1/self.beta) #**self.beta
            print(torch.min(weights))
            print(torch.max(weights))
