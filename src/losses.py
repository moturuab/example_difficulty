...
class WeightedCrossEntropyLoss(nn.CrossEntropyLoss):
    ...
    def weights(self, outputs, encoded_targets, m=0):
        ...
        #weights = torch.exp(-((correct_outputs/max_outputs - self.beta)**2))
        if not m:
            weights = ((1-self.alpha)*correct_outputs/max_outputs)
        else:
            weights = ((1-self.beta)*correct_outputs/max_outputs)
        return weights
