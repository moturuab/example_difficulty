...
class WeightedCrossEntropyLoss(nn.CrossEntropyLoss):
    ...
    def weights(self, outputs, encoded_targets, m=0):
        ...
        if not m:
            ...
            weights = self.sigmoid(self.alpha**2*correct_outputs - self.alpha*max_outputs) #**self.alpha
            ...
            weights = torch.exp(-(-(self.beta*correct_outputs - max_outputs))**2/2) #**self.beta
            print(torch.min(weights))
            print(torch.max(weights))
